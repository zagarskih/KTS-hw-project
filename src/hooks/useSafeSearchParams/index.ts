...
export const useSafeSearchParams = <Schema extends z.ZodSchema<Record<string, unknown>>>(schema: Schema) => {
  type ActionOrValue = ((prev: z.infer<Schema>) => z.infer<Schema>) | z.infer<Schema>;
  ...
  const state: z.infer<Schema> = useMemo(() => parseSearchParams(searchParams, schema), [searchParams, schema]);
